...
```
403 Client Error: Forbidden for url: https://search.jrb.nz/search?q=...&format=json
```
...
- URL: `https://search.jrb.nz/search?q=<query>&format=json`
...
2. **Verify SearXNG is accessible**:
   ```bash
   curl "https://search.jrb.nz/search?q=test&format=json" \
...
SEARXNG_QUERY_URL: "https://search.jrb.nz/search?q=<query>&format=json"
...
curl "https://search.jrb.nz/search?q=weather&format=json" \
  -H "User-Agent: OpenWebUI/1.0"
